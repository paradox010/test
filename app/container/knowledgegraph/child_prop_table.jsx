...
export default class Child02 extends React.Component{
    ...
    componentDidMount(){
        ...
        const {attrList, relationList, roleList} = this.props.dataSource.entityTreeSlecetItem;
        ...
        if (this.props.isRelation) {
            data2 = relationList.map(item=>{
                item.key = item.id;
                return item
            })
        }else {
            data2 = roleList.map(item=>{
                item.key = item.id;
                return item
            })
        }
    ...
    componentWillReceiveProps(nextProps){
        // this.cacheData = nextProps.dataSource.entityTreeSlecetItem;
        // const {attrList, relationList, roleList} = nextProps.dataSource.entityTreeSlecetItem;
        // const data1 = attrList.map(item=>{
        //     item.key = item.id;
        //     return item
        // })
        // const list = this.props.isRelation?relationList:roleList;
        // const data2 = list?list.map(item=>{
        //     item.key = item.id;
        //     return item
        // }):[]
        //
    ...
    save = (key) => {
        ...
        if (target) {
            ...
            if (key === 'none') {
                ...
                //update
                this.props.onSave({data: this.cacheData, item: rest, CB:(id)=>{
                    if (!id) {
                        target.editable = true;
                        this.setState({ tableData: newData});
                        return
    ...
    handleCacheData = newData => {
        ...
        if (this.props.isRelation) {
            ...
            this.cacheData.roleList = relationList;
        }
    }
    ...
    delete = (record) => {
        if (!this.state.isEdit) {
            const listName = record.propType === '数值'?'attrList':this.props.isRelation?'relationList':'roleList';
            let deleteIndex;
    ...
    handleAddEntityProp = () => {
        ...
        this.setState({
            tableData: [tableAddNewItem,...this.state.tableData ],
            isEdit: true,
            treeSelectValue: []
        });
    }
    ...
    handleVisibleChange = async() => {
        this.recommendList = await knowledgegraphApi[this.props.isRelation?'selectRecommendRelationOrAttribute':'selectRecommendRoleOrAttribute']({pid:this.props.dataSource.entityTreeSlecetPValue});
    ...
    render(){
        return(
            <div style={{height:"83%"}}>
                {/*<Divider />*/}
                <NewBtn disabled={this.state.isEdit} text='创建属性' style={{marginBottom: 10, marginRight:10}} onClick={this.handleAddEntityProp} icon='plus' type='D' />
                ...
                <Popover
                    ...
                    content={
                        <div style={{width:300,maxHeight:"350px"}} className="selectList">
                            {
                                this.state.checkboxOpts.map(opts=>{
                                    return <div key={opts.key}>{
                                        opts.child.length?<span>
                                            <div style={{fontWeight: 'bold',marginBottom: 5,height:30,borderBottom:"1px solid #cdcdcd"}}>请勾选需要添加的属性</div>
                                            <CheckboxGroup className="checkboxList" options={opts.child} onChange={e=>{this.handleCheckBoxChange(e, opts.key)}} />
                                        </span>:null
                                    }
                                    </div>
                                })
                            ...
                            <div style={{width:'100%', textAlign:'right', marginTop: 40}}>
                                <Button style={{marginRight:20}} onClick={this.cancelModal}>取消</Button>
                                <Button type="primary" className="primaryColor" onClick={this.handleQuickAddEntityProp}>确定</Button>
                            </div>
                        </div>
                    }
                    visible={this.state.visible}
                    ...
                    trigger="click"
                >
                    <NewBtn disabled={this.state.isEdit} text='快速添加' style={{marginBottom: 10}}  icon='plus' type='D' />
                    {/*<Button disabled={this.state.isEdit} type="primary" style={{marginBottom: 10}}>快速添加</Button>*/}
                </Popover>
                <Table className="propertyList flexTable"
