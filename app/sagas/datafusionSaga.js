...
function* initDataFusion(){
    ...
    if (treeData.databaseSource.length) {
        ...
        const treeNodeDetail = yield call(datafusionApi.getTreeNodeDetail,{mongoId:mongoId,pageSize:6,page:1});
...
function* pageChange(args){
    const treeNodeDetail = yield call(datafusionApi.getTreeNodeDetail,args.args);
    console.log('翻页后得到数据：', treeNodeDetail);
    yield put({type: 'datafusion/SET_TREENODE_DETAIL', payload: treeNodeDetail})
}
...
function* db_add_mappingConfNext(){
    ...
    if (nextMappingConfStep === 1) {
        ...
        // 上面步骤条+1
        yield put({type: 'datafusionChildDbAdd/ONLOAD_STEP'+ nextMappingConfStep + '_TREE_DATA', status: 'next', payload: {treeData:[{title: "名称", key: "_name"},...treeData],treeData2}});
        ...
        const {value: selectValue, name: selectLabel, treeData} = mappingSelectData[mappingConfStep][0] || {value:'', label: '', treeData: []};
...
function* changeTreeSelect({args}){
    const treeNodeDetail = yield call(datafusionApi.getTreeNodeDetail,{mongoId:args.value,page:1,pageSize:6});
...
function* hanleNewTagSave({args}){
    ...
    if (step === 1 || step === '1') {
        ...
        args.CB()
        const treeNodeDetail = yield call(datafusionApi.getTreeNodeDetail,{mongoId:lastItem.value,page:1,pageSize:6});
        yield put({type: 'datafusion/SET_TREENODE_DETAIL', payload: treeNodeDetail})
    }
}
...
function* handleMCSelectChange({args:{value,index}}){
    ...
    if (mappingConfStep === 1) {
        ...
        yield put({type: 'datafusionChildDbAdd/ONLOAD_STEP'+ mappingConfStep + '_TREE_DATA', status: 'next', payload: {treeData:[{title: "名称", key: "_name"}, ...treeData]}});
...
function* getDocTypeDb(params){
    const result = yield call(datafusionApi.getDocType,params.args);
    ...
    yield put({type:'datafusionChildDbAdd/GET_DOC_TYPE_LIST_OK',payload:payload})
}
...
function* getDocType(params){
    ...
    yield put({type:'datafusionChildDmlAdd/GET_DOC_TYPE_LIST_OK',payload:payload})
}

function* getDocChildData({args:{treeNode,newTreeData}}){

    const loadData = yield call(datafusionApi.selectDocByPid, {id: treeNode.props.value});
    if (loadData.length) {
        newTreeData.map((item,index)=>{
            if(item.value===treeNode.props.value){
               item.children=loadData.map((item,index)=>{
                   return {
                       title: item.name,
                       key: `${treeNode.props.eventKey}-${index}`,
                       value: item.id,
    ...
    yield put({type:'datafusionChildDmlAdd/GET_DOC_TYPE_LIST_OK',payload:newTreeData})
    yield put({type:'datafusionChildDbAdd/GET_DOC_TYPE_LIST_OK',payload:newTreeData})
}
function* watchCreateLesson() {
    yield[
        ...
        takeLatest('datafusion/saga/PAGE_CHANGE',pageChange),
        ...
        takeLatest("datafusionChildDbAdd/saga/GET_DOC_TYPE",getDocTypeDb),
        ...
        takeLatest("datafusionChildDmlAdd/saga/GET_DOC_TYPE",getDocType),
        takeLatest('datafusionChildDmlAdd/saga/GET_DOC_CHILD_TYPE',getDocChildData),
