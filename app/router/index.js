...
const Label = Loadable({
    loader: () => import(/* webpackChunkName: 'label' */'../container/label/index'),
    loading: MyLoadingComponent
});
//用户
...
//没选领域
const AsyncNoDmain = Loadable({
    loader: () => import(/* webpackChunkName: 'error_nodomain' */'../component/error/nodomain'),
    loading: MyLoadingComponent
});
...
const WrapApp = (props)=>{
    return (
        <App {...props}>
            <Switch>
                <Route path={`${props.match.path}`} exact component={AsyncTypeSystem} />
        ...
        return (
            <HashRouter>
                <Switch>
                    ...
                    <Route path="/error/nodomain" component={AsyncNoDmain} />
                    <Route path="/supermind" component={WrapApp} />
                    <Route path="/domain" exact component={AsyncHomePage} />
                    <Route path="/label/:search" component={Label} />
                    <Redirect to="/domain" />
                </Switch>
            </HashRouter>
        );
